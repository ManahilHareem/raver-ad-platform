...
export function MarkdownRenderer({ content, className, isUser = false }: MarkdownRendererProps) {
  // Pre-process content to strip out technical system context/instructions that shouldn't be visible to the user
  let displayContent = content;
  
  if (displayContent.includes("### ATTACHED MEDIA CONTEXT ###")) {
    displayContent = displayContent.split("### ATTACHED MEDIA CONTEXT ###")[0];
  }
  
  if (displayContent.includes("### SYSTEM INSTRUCTION")) {
    displayContent = displayContent.split("### SYSTEM INSTRUCTION")[0];
  }
  
  return (
    <div className={cn(
      "markdown-container text-[13px] max-w-[400px] leading-relaxed no-scrollbar",
